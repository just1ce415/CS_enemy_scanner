...
def get_data_from_steam(profile_link: str):
    ...
    search_url = (
        f"{base_url}?appid={appid}&key={api_key}&steamid={playerid}&format=json"
    )
    response = requests.get(search_url)   # WITHOUT MAP?
...
def kill_death(json_data):
    ...
    """
    total_kills = json_data["playerstats"]["stats"][0]["value"]    # GAME?
...
def average_damage_per_round(json_data):    # SAME HERE
    """
    Returns your ADR
    """
